...
export default function BuildNexusScreen({ navigation }) {
  ...
  const handleCreateNexus = async () => {
    ...
    try {
      ...
      // 2) prepare payload to match backend controller
      const body = {
        name,
        type: "default",          // or whatever type you use
        icon: avatarImage || selectedEmoji,  // backend expects 'icon' string
        description: "",          // add description field later if needed
        is_public: true,          // or false based on your UI
      };
      ...
      const response = await fetch(`${BASE_URL}/nexus`, {
        ...
        headers: {
          ...
          Authorization: `Bearer ${token}`,  // auth middleware -> req.user
        },
      ...
      if (!response.ok || !json.success) {
        console.log("Create nexus error:", json);
        throw new Error(json.message || "Failed to create nexus");
      }

      const createdNexus = json.data;

      // 4) move to next screen with created nexus data
      setLoading(false);
      navigation.navigate("Share_Profile", {
        nexus: createdNexus,
        name: createdNexus.name,
        icon: createdNexus.icon,
        image: avatarImage, // keep local image if you want to show it
      });
    } catch (err) {
      setLoading(false);
      console.log("Create nexus exception:", err);
